...
drive = {
	...
	setup: function() {
		...
		if (typeof browser !== 'undefined') {
			...
			if (!browser.executeAndStore) {
				...
				browser.addCommand('executeAndStore', function(script, resultKey) {

					return this.execute(script).then(function(data, err) {

						if (err) {
							throw new Error(err);
						} else {
							browser.execute('localStorage.setItem("' + resultKey + '", "' + data.value + "" + '")');
						}
	...
	execute: function(args) {
		...
		switch (args.execute.action) {
			...
			case 'switchTab':
				...
				execTest('switching to tab index: ' + args.execute.focusIndex, function(done) {
					...
					browser
						...
						.then(function(data) {
							...
							if (data) {
								...
								browser
									.switchTab(tabList[args.execute.focusIndex])
									.then(function() {
										browser.call(done);
									});
			...
			case 'closeTab':
				...
				execTest('closing the current tab and focusing on tab index: ' + args.execute.focusIndex, function(done) {
					...
					browser
						...
						.then(function(data) {
							...
							if (data) {
								...
								browser
									.close(tabList[args.execute.focusIndex])
									.then(function() {
										browser.call(done);
									});
		...
		if (actionDescription && actionMethod && actionArgs) {

			execTest(actionDescription, function(done) {

				var fn = browser[actionMethod];

				fn.apply(browser, actionArgs).then(function() {
					browser.call(done);
				});
	...
	/**
	 * This method allows the developer to verify the title of the page.
	 * @param args
	 * @returns {Array}
	 */
	...
	verifyPageTitle: function(args) {
		...
		it('the "' + args.assertKey + '" of this page ' + getChaiAssertString(args.mode) + ' "' + args.assert + '"', function(done) {
			...
			browser.title(function(err, data) {
	...
	/**
	 * An easy verification method to count the number of elements on a page.
	 * @param args
	 * @returns {Array}
	 */
	...
	verifyCount: function(args) {
		...
		it('the number of "' + args.selector + '" elements on the page ' + getChaiAssertString(args.mode) + ' "' + args.assert + '"', function(done) {
			...
			browser.elements(args.selector, function(err, data) {
	...
	/**
	 * A method for running a simple snippet of JavaScript and verifying the result.
	 * @param args
	 * @returns {Array}
	 */
	...
	verifyScriptResult: function(args) {
		...
		it('evaluating "' + args.selector + '" returns a result that ' + getChaiAssertString(args.mode) + ' "' + args.assert + '"', function(done) {
			...
			browser.execute(args.selector, function(err, data) {
	...
	go: function(obj) {
		...
		if (!obj.config.description) {
			throw new Error('Your test configuration is missing a description.\n"config": {\n  "description": "This is a description." \n}');
		}

		if (!obj.config.suites) {
			throw new Error('Your test configuration is missing a "suites" array. \n"config": {\n  "suites": [\n    {\n      "description": "Verify colors",\n      "tests": ["color-styles"],\n      "viewports": {\n        "large": true\n      }\n    }\n  ] \n}');
		}

		if (!Array.isArray(obj.config.suites)) {
			throw new Error('Your test configuration needs a "suites" array.  Are you using an object?\n"config": {\n  "suites": [\n    {\n      "description": "Verify colors",\n      "tests": ["color-styles"],\n      "viewports": {\n        "large": true\n      }\n    }\n  ] \n}');
		}
		...
		describe(obj.config.description, function() {
			...
			obj.config.suites.forEach(function(testSuite) {
				...
				if (!testSuite.description) {
					throw new Error('Your suite is missing a description.\n"suites": [\n  {\n    "description": "Verify colors",\n    "tests": ["color-styles"],\n    "viewports": {\n      "large": true\n    }\n  }\n]');
				}

				if (!testSuite.tests) {
					throw new Error('Your suite is missing a "tests" array.\n"suites": [\n  {\n    "description": "Verify colors",\n    "tests": ["color-styles"],\n    "viewports": {\n      "large": true\n    }\n  }\n]');
				}

				if (!Array.isArray(testSuite.tests)) {
					throw new Error('Your suite needs to have a "tests" array. Are you using an object?\n"suites": [\n  {\n    "description": "Verify colors",\n    "tests": ["color-styles"],\n    "viewports": {\n      "large": true\n    }\n  }\n]');
				}

				if (!testSuite.viewports) {
					throw new Error('Your suite is missing a "viewports" object.\n"suites": [\n  {\n    "description": "Verify colors",\n    "tests": ["color-styles"],\n    "viewports": {\n      "large": true\n    }\n  }\n]');
				}
						...
						describe('Rendered in a ' + viewport + ' viewport', function() {
							...
							describe(testSuite.description, function() {
								...
								testSuite.tests.forEach(function(test) {
									...
									if (!obj[test].steps) {
										throw new Error('Your test object is missing a "steps" array.\n"steps": [\n  {\n    "selector": ".panel",\n    "verify": [\n      {\n        "method": "verifyCount",\n         "asserts": {\n           "count": 13\n        }\n      }\n    ]\n  }\n]');
									}
